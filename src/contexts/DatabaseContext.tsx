...
export function DatabaseProvider({ children }: { children: ReactNode }) {
  ...
  const testSupabaseConnection = useCallback(async () => {
    ...
    try {
      ...
      if (!supabaseUrl || !supabaseKey || supabaseUrl.includes('placeholder') || supabaseKey.includes('your-anon-key')) {
        setIsConnected(false);
        setConnectionError('Supabase not configured. Please click "Connect to Supabase" button to set up your database connection.');
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    reconnectTimeoutRef.current = setTimeout(() => {
      if (!isConnected) testSupabaseConnection();
    }, retryDelay);
